 ...
 */
class ApiQueueEntry(private val request: Request?, private val apiCallback: ApiCallback?) {
    ...
    fun execute() {
        LoggerHelper.logInfo(debugInfo + "Making a request: " + request?.getURL(HypixelAPI))

        when (request?.requestType) {
            RequestType.BOOSTERS -> executeBoosterRequest()
            RequestType.FRIENDS -> executeFriendRequest()
            RequestType.PLAYER -> executePlayerRequest()
        }
    ...
    companion object {
        ...
        private val debugInfo = "[API][QueueEntry]"
    }
