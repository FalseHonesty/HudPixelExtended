...
object VZBalanceModularGuiProvider : CoinCounterModularGuiProvider() {
    private val type = Type.Total
    ...
    val renderingString: String
        get() {
            if (this.shouldDisplay) {
                when (this.type) {
                    VZBalanceModularGuiProvider.Type.Negative -> return NEGATIVE_COINS_DISPLAY_TEXT + -this.coins
                    VZBalanceModularGuiProvider.Type.Total -> if (this.coins < 0) {
    ...
    override fun onChatMessage(textMessage: String, formattedMessage: String) {
        ...
        if (textMessage.startsWith("[VampireZ]: Purchased ") && (textMessage.toLowerCase().contains("blood") || textMessage.toLowerCase().contains("gold"))) {
            ...
            // get the amount
            var amountStr = textMessage.replace("[VampireZ]: Purchased ", "")
            amountStr = amountStr.substring(0, amountStr.indexOf(" "))
            var amount = 0
            try {
                amount = Integer.valueOf(amountStr)!!
            } catch (e: Exception) {
                HudPixelMod.logInfo("Failed to parse amount out of VampireZ purchase message. Ignoring.")
            }
