...
object WallsKillCounterModularGuiProvider : SimpleHudPixelModularGuiProvider(), McColorHelperJava {
    ...
    override fun onChatMessage(textMessage: String, formattedMessage: String) {
        when (this.trackedType) {
            WallsKillCounterModularGuiProvider.KillType.Normal ->
                // I hope killed by includes all possible deaths
                // known common messages:
                // was killed by
                // was shot and killed by
                if (textMessage.contains("killed by " + FMLClientHandler.instance().client.session.username)) {
                    this.kills++
                }
            WallsKillCounterModularGuiProvider.KillType.Assists ->
    ...
    val renderingString: String
        get() {
            when (trackedType) {
                WallsKillCounterModularGuiProvider.KillType.Normal -> return KILL_DISPLAY + this.kills
                WallsKillCounterModularGuiProvider.KillType.Assists -> return ASSISTS_DISPLAY + this.kills
            }
            return ""
        }
