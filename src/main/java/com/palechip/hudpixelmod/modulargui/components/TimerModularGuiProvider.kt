...
object TimerModularGuiProvider : HudPixelModularGuiProvider(), McColorHelperJava {
    private var tickTime: Long = 0
    private var runningTime = "00:00"
    private var gameStartedTime: Long = 0
    private var gameStarted: Boolean = false
    private val minutes = 0
    private val seconds = 0

    override fun doesMatchForGame(): Boolean {
        return GameDetector.currentGameType !== GameType.UNKNOWN
    }

    val renderingString: String
        get() = TIME_DISPLAY_MESSAGE + runningTime
    ...
    override fun onTickUpdate() {
        if (gameStarted && !GameDetector.isLobby()) {
            val timeBuff = System.currentTimeMillis() - gameStartedTime
            val sMin: String
